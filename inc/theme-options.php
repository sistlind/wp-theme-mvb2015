...
function theme_options_add_page() {
	add_theme_page(__('Theme-Optionen', 'tf2013'), __('Theme-Optionen', 'tf2013'), 'edit_theme_options', 'theme_options', 'theme_options_do_page');
}
						...
						if (isset($setoptions['tf2013_theme_options'][$tab]['fields'])) {
							foreach ($setoptions['tf2013_theme_options'][$tab]['fields'] as $i => $value) {
								...
								if (isset($value['liste']))
									$liste = $value['liste'];
								if (isset($value['mark_option']) && $value['mark_option'] == 1)
									$mark_option = 1;
								if ($type == 'section') {
									...
									if ($type == 'bool') {
										echo "\t\t\t";
										echo "<input id=\"$name\" name=\"tf2013_theme_options[$name]\"
...
cols=\"30\" rows=\"10\" name=\"tf2013_theme_options[$name]\">";
										...
										?>
										<input type="hidden" name="tf2013_theme_options[<?php echo $name; ?>]" id="file_<?php echo $name; ?>"
											   value="<?php if (isset($options[$name])) echo sanitize_key($options[$name]); ?>" />
										<input type="text" name="tf2013_theme_options[<?php echo $name; ?>_url]" id="file_<?php echo $name; ?>_url"
											   value="<?php if (isset($options[$name])) echo wp_get_attachment_url(esc_attr($options[$name])); ?>" />
										<input class="button" name="file_button_<?php echo $name; ?>" id="file_button_<?php echo $name; ?>" value="<?php _e('Datei hinzuf&uuml;gen', 'tf2013'); ?>" />
										<small><a href="#" class="file_remove_<?php echo $name; ?>"><?php _e("Datei entfernen", 'tf2013'); ?></a></small>
										<br><label for="tf2013_theme_options[<?php echo $name; ?>]"><?php echo $label; ?></label>
								</div><script>
								...
								if ((isset($options[$name])) && esc_url($options[$name])) {
									...
									_e('Kein Bild ausgew&auml;hlt', 'tf2013');
								}
								?>
						</div>
						<input type="hidden" name="tf2013_theme_options[<?php echo $name; ?>_id]" id="image_<?php echo $name; ?>_id"
							   value="<?php if (isset($options[$name . "_id"])) echo sanitize_key($options[$name . "_id"]); ?>" />
						<input type="text" name="tf2013_theme_options[<?php echo $name; ?>]" id="image_<?php echo $name; ?>" value="<?php if (isset($options[$name])) echo esc_attr($options[$name]); ?>" />
						<input class="button" name="image_button_<?php echo $name; ?>" id="image_button_<?php echo $name; ?>" value="<?php _e('Bild hinzuf&uuml;gen', 'tf2013'); ?>" />
						<small><a href="#" class="image_remove_<?php echo $name; ?>"><?php _e("Bild entfernen", 'tf2013'); ?></a></small>
					<?php if (isset($value['default']) && (filter_var($value['default'], FILTER_VALIDATE_URL))) { ?>
							<small><a href="#" class="image_reset_<?php echo $name; ?>"><?php _e("Standard wiederherstellen", 'tf2013'); ?></a></small>
					<?php } ?>
						<br><label for="tf2013_theme_options[<?php echo $name; ?>]"><?php echo $label; ?></label>
					</div><script>
						jQuery(document).ready(function () {
						...
						jQuery(document).ready(function () {
							jQuery('.image_remove_<?php echo $name; ?>').click(function () {
								...
								jQuery('.showimg_<?php echo $name; ?>').html('<?php _e('Kein Bild ausgew&auml;hlt', 'tf2013'); ?>');
								return false;
							});
						});
					<?php if (isset($value['default']) && (filter_var($value['default'], FILTER_VALIDATE_URL))) { ?>
...
// First Dir only
					...
					echo "\t\t\t";
					echo "<select id=\"$name\" name=\"tf2013_theme_options[$name]\">\n";
					foreach ($liste as $i => $value) {
						echo "\t\t\t\t";
						echo '<option value="' . $i . '"';
						if ($i == $options[$name]) {
					...
					foreach ($liste as $entry => $listdata) {
						...
						?>
						<label for="tf2013_theme_options[<?php echo $name ?>][<?php echo $entry ?>]" >
							<input type="checkbox"
								   id="tf2013_theme_options[<?php echo $name ?>][<?php echo $entry ?>]"
								   name="tf2013_theme_options[<?php echo $name ?>][<?php echo $entry ?>]"
	...
	<p class="submit">
		<input type="submit" class="button-primary" value="<?php _e('Speichern', 'tf2013'); ?>" />
	</p>
	</form>
	</div>
...
// $options= $saved;
	...
	if ((isset($input['position_sidebarbottom'])) && ($input['position_sidebarbottom'] == 1) && ($input['aktiv-dynamic-sidebar'] == 1)) {
	...
	if (isset($input['reset_options']) && ($input['reset_options'] == 1)) {
		delete_option('tf2013_theme_options');
	}
	return $output;
}
